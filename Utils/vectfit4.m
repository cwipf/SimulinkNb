...
if opts.spy2 == 2
    ...
    semilogx(freq, 20*log10(abs(f)),'ro',...
             ...
             freq, 20*log10(abs(10*diff)),'g-o');
    set(top,'XTickLabel',[]);
    %set(top,'GridLineStyle','--');
    %axis tight
    hihi = 20*log10(1.3*max(abs(f)));
    lolo = 20*log10(0.1*min(abs(f)));
    axis([min(freq) max(freq) lolo hihi])
    grid
    ylabel('Mag [dB]');
    ...
    semilogx(freq, angle(f)*180/pi,'ro',...
             ...
             freq, 10*(angle(fit)-angle(f))*180/pi,'g-o');
    set(bottom,'YTick',[-180:45:180]);
    axis([min(freq) max(freq) -181 181]);
    grid
    %set(bottom,'GridLineStyle','--');
    legend('TF Data','Fit','Residual x10')
    xlabel('Frequency [Hz]');
    ylabel('Phase [deg]')
end
