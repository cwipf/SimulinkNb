...
%   "NbNoiseSource" or "NbNoiseSink".  The model is expected to contain at
%   least one source, and exactly one sink.
%
...
load_system(mdl);
nbNoiseSources = find_system(mdl, 'RegExp', 'on', 'Tag', '[Nn][Bb][Nn]oise[Ss]ource');
nbNoiseSinks = find_system(mdl, 'RegExp', 'on', 'Tag', '^[Nn][Bb][Nn]oise[Ss]ink');
...
%% Evaluate each NbNoiseSource block's asd, and set up calibration TFs
...
for n = 1:numel(nbNoiseSources)
    ...
    % set to NOT evaluate the expr automatically.  This way, the noise
